...
- `/daily_review` health rule includes runner-state readiness: latest `runner_status` of `failed` or `unknown` is treated as `attention_needed` even when pnl/outcome sections are available.
...
- Run-level: `/decision_note scope=run run_id=321 source_command=/daily_review human_action=observe confidence=medium note=Daily review checked; no action.`
- Stock-level: `/decision_note scope=stock run_id=321 stock_id=0700.HK source_command=/risk_review system_signal=buy_signal human_action=investigate confidence=low note=Need risk review first.`
...
User-supplied required fields:
- `scope`
- `run_id`
- `human_action`
- `note`
- `source_command`
- `stock_id` only when `scope=stock`

System-generated required fields:
- `created_at` (generated by future runtime command at record creation time)
- `operator_user_id_hash_or_label` when available/applicable

Clarification:
- Operator should **not** manually supply `created_at` in Telegram command text.
...
## 2026-05-13 — Step 135G Mini App first-view IA cleanup contract
- Mini App default Today first screen is a compact single-hero decision-support summary.
- Full diagnostics/runtime metadata and full PnL/risk breakdown move to dedicated secondary tabs.
- This is presentation/IA refinement only; no new trading capability, no write path expansion, no broker/live/real-money execution behavior.
