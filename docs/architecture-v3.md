...
- Daily summary payload contract is versioned with explicit evolution guardrails: a current version constant, an allowlisted supported-version set, centralized schema-dispatch renderer mapping, and fail-fast handling for unsupported/misconfigured versions.
