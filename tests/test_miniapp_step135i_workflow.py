...
def test_journal_create_first_collapsed_sections():
    ...
    assert '<details id="journal-context"><summary>決策參考資料</summary>' in HTML
    assert 'id="journal-context-content"' in HTML
    assert '<details id="journal-snapshot-review"><summary>最近紀錄</summary>' in HTML
    assert 'id="journal-snapshot-content"' in HTML
    assert '<details id="journal-outcome-review"' in HTML
