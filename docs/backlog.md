...
1. **Step 135D — HK Equity Strategy Research Reference docs-of-record (docs-only)**
   ...
   - Define horizon policy (short=observation only for now; medium/long=current implementation focus), HK-market characteristics, strategy families, desk mapping, scorecard/risk-gate framework, phased rollout, and hard boundaries.
...
- **Step 87 completed:** added `LocalArtifactMiniAppReadDataProvider` as first concrete `latest_system_run` provider using backend-local bounded JSON artifact path (`MINIAPP_LATEST_SYSTEM_RUN_ARTIFACT_PATH`) with bounded unavailable fallback on missing/invalid artifact and no Supabase production read / no frontend fetch wiring / no write-order-execution path.
...
- **Step 92B-1 completed:** `/latest_system_run` now uses HKT display labels (`data_timestamp_hkt`, `updated_at_hkt`) for operator-facing output while preserving UTC/ISO storage semantics; `latest_system_runs` upsert freshness is hardened by including timezone-aware UTC `updated_at` in payload. latest-state-only boundary is unchanged (not an audit ledger), and Mini App fetch remains deferred.
...
- **Step 127 (merged):** baseline market-data acceptance semantics landed for Decision Context + `/daily_review`.
